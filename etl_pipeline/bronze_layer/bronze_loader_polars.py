...
class BronzeLoaderPolars:
    ...
    #         [
    #             pl.lit(current_time).alias("_valid_from"),
    #             pl.lit(None).cast(pl.Utf8).alias("_valid_to"),
    #             pl.lit(True).alias("_is_current"),
    #             pl.lit(1).alias("_record_version"),
    #             pl.lit(datetime.now()).alias("_load_timestamp"),
    #             pl.lit(datetime.now().date()).alias("_ingestion_date"),
    #             pl.lit(str(datetime.now())).alias("_source_file_path"),  # Temporary
    # change this <---------------------------->
    #             pl.lit("combined").alias("_source_file_name"),
    #             pl.lit("detailed").alias("_data_granularity"),
    #         ]
    #     )
    ...
    #             pl.lit("detailed").alias("_data_granularity"),  # DELETE
    #         ]
    #     )
    ...
    def prepare_scd2_data(
        ...
        scd2_df = df_fixed.with_columns(
            [
                pl.lit(current_time).alias("_valid_from"),
                pl.lit("").cast(pl.Utf8).alias("_valid_to"),  # Changed from None to ""
                pl.lit(True).alias("_is_current"),
                pl.lit(1).alias("_record_version"),
                pl.lit(datetime.now()).alias("_load_timestamp"),
                pl.lit(datetime.now().date()).alias("_ingestion_date"),
    ...
    def implement_simple_scd2(
        ...
        try:
            ...
            if primary_keys:
                ...
                existing_current = existing_df.filter(pl.col("_is_current") == True)
    ...
    #                         pl.lit(data_type).alias("_data_granularity"),
    #                     ]
    #                 )
