...
class PostgreSQLLogger:
    ...
    def __init__(self, db_config=None):
        ...
        try:
            ...
            logger.info(
                f"Connected to PostgreSQL database: {self.db_config['database']}"
            )
    ...
    def __init__(self, db_logger, config=None):
        ...
        if config:
            ...
            self.config = {
                ...
                "pipeline": {
                    "clean_orphaned": os.getenv("CLEAN_ORPHANED", "true").lower()
                    == "true",
        ...
        try:
            ...
            logger.info(
                f"Docker client initialized, targeting container: {self.pipeline_container}"
            )
        except Exception as e:
    ...
    def _trigger_pipeline_processing(self, file_path: str, event_type: str):
        ...
        try:
            ...
            try:
                ...
                logger.error(
                    f"Pipeline container '{self.pipeline_container}' not found"
                )
                self.db_logger.log_event(
                    "ERROR",
            ...
            if result.exit_code == 0:
                ...
                error_msg = (
                    result.output.decode("utf-8") if result.output else "Unknown error"
                )
...
def main():
    ...
    parser.add_argument("--watch-dir", help="Override watch directory from config")
    parser.add_argument(
        "--container", help="Override pipeline container name from config"
    )
    parser.add_argument(
        "--delay", type=int, help="Override processing delay from config (seconds)"
    )
    parser.add_argument("--db-host", help="Override database host from config")
    parser.add_argument(
        "--db-port", type=int, help="Override database port from config"
    )
    parser.add_argument(
        "--show-logs", action="store_true", help="Show recent logs before starting"
    )
    ...
    try:
        ...
        if args.show_logs:
            ...
            if logs:
                for log in logs:
                    print(
                        f"{log['timestamp']} [{log['level']}] {log['message']} - {log.get('file_path', '')}"
                    )
            else:
